...
let walletCoin: number = 25000; //소지금
let inputCoin: number = 10000; // 입금액
let totalPrice: number = 0; //총 금액 (총금엑 += 음료가격 * 음료 개수)
let itemNum: number = drinks.length; // 음료의 개수
...
const pocket = document.querySelector<HTMLDivElement>(".pocket");
...
function update(): void{
    // 잔액을 찍어주는 코드
    const $balanceResult = machineFunc?.querySelector(".balance_result");
    if ($balanceResult instanceof HTMLSpanElement){
        $balanceResult.textContent = balance.toLocaleString() + "원";
    }

    // 소지금을 찍어주는 코드
    const $walletCoin = pocket?.querySelector(".wallet_coin");
    ...
    // 총금액을 찍어주는 코드
    const $totalPrice = pocket?.querySelector(".total-price");
...
// 입금 기능 함수 deposit()
function deposit() {
    const inputElem = machineFunc?.querySelector(".input-payment");

    if (inputElem instanceof HTMLInputElement){
        inputCoin = parseInt(inputElem?.value);
    }
...
function addtoSelectedList(id: number): void {
    ...
    if (selectList !== undefined){
        [...selectList].forEach(i => {
            ...
            if (countTag !== null && getDrinkList.includes(countTag.id)){
                const tempElem = pocket?.querySelector(`#${countTag?.id}`);
...
function cancel(tag: HTMLLIElement){
